...
//	char buf[128]={0};
	path=getcwd(NULL,0);
	DIR*dir=opendir(path);//open返回指向DIR结构体的指针，该指针可被几个与目录有关的文件使用，返回目录的相关信息
	struct dirent*dinfo=NULL;//dinfo结构体保存目录下某文件的信息(相对于目录起始位置的偏移，文件类型，文件名等)，想得到文件的具体信息，用stat函数获取
